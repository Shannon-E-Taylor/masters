        ...
        for gene in self.genes.values():
            ...
            for input_state, x in np.ndenumerate(state_space.array):
                ...
                for i in range(num_genes):
                    state_dict[state_dict.keys()[i]] = input_state[i]
    ...
    def calculate_stable_states(self):
        ...
        for input_state, x in np.ndenumerate(state_space.array):
            ...
            for i in range(len(self.state_space_dims)):
                gene_state_space = self.state_spaces[state_space.inputs[i]]
    ...
    def print_stable_states(self):
        ...
        for i in range(len(self.stable_states)):
            ...
            for j in range(len(stable_state)):
                print self.genes.keys()[j] + '\t%i' % stable_state[j]
...
class Tissue(object):
    ...
    def __init__(self, l, n, net):
        ...
        self.num_cells = n
        self.network = net
        self.genome = self.network.genes.keys()
        self.cells = []
        for i in range(n):
    ...
    def extract_state(self, tissue_object, var, simulation_title, timepoint):
        assert simulation_title in tissue_object.simulations
        genelist = tissue_object.simulations[simulation_title].gene_labels
        state = getattr(tissue_object.simulations[simulation_title], var)[timepoint,:,:]
        state = np.swapaxes(state, 0,1) #swap axes so that gene is 1st 
        state_zip = zip(genelist, state) 
        state_dict = dict(state_zip)
        return state_dict

    def set_default_params(self): 
        self.set_uniform_delays(6,6)
        self.set_delays("g1", 18, 30)
        self.set_delays("g2", 18, 30)
        self.set_delays("cad", 6, 80)
        self.set_delays("opa", 80, 6)

    def set_initial_conditions(self, tissue_object, timepoint, simulation_title):
        assert simulation_title in tissue_object.simulations
        rna_history = self.extract_state(tissue_object, "rna_history", simulation_title, timepoint)
        rna_age_history = self.extract_state(tissue_object, "rna_age_history", simulation_title, timepoint)
        protein_history = self.extract_state(tissue_object, "protein_history", simulation_title, timepoint)
        protein_age_history = self.extract_state(tissue_object, "protein_age_history", simulation_title, timepoint)
        for key in rna_history: 
            self.set_rna_state(key, rna_history[key])
        for key in rna_age_history: 
            self.set_rna_ages(key, rna_age_history[key])
        for key in protein_history: 
            self.set_protein_state(key, protein_history[key])
        for key in protein_age_history: 
            self.set_protein_ages(key, protein_age_history[key])


    def set_initial_conditions_given(self, rna_history, rna_age_history, protein_history, protein_age_history, genelist):
        for key in genelist: 
            self.set_rna_state(key, rna_history[key])
            self.set_rna_ages(key, rna_age_history[key])
            self.set_protein_state(key, protein_history[key])
